...
if settings.CACHE_ENABLED:
    rewrite_cache = TTLCache(maxsize=settings.CACHE_MAX_SIZE, ttl=settings.CACHE_TTL_SECONDS)
    logger.info(f"In-memory cache enabled: maxsize={settings.CACHE_MAX_SIZE}, ttl={settings.CACHE_TTL_SECONDS}s")
else:
...
class RewriteService:
    ...
    def _rewrite_decorator(self, func):
        if settings.CACHE_ENABLED and rewrite_cache is not None:
            ...
            return cached(cache=rewrite_cache, key=lambda self, text, style: hashkey(text, style))(func)
        else:
    ...
    async def rewrite_text(self, text: str, style: str) -> str:
        ...
        async def _perform_rewrite(inner_self, inner_text: str, inner_style: str) -> str:
            logger.info(f"Performing rewrite for style '{inner_style}' (Cache {'hit' if settings.CACHE_ENABLED and hashkey(inner_text, inner_style) in rewrite_cache else 'miss/disabled'})")
            # In a real scenario, add more robust error handling here
            try:
                rewritten_text = await inner_self.llm_adapter.rewrite(inner_text, inner_style)
                return rewritten_text
                 ...
                 logger.error(f"Unexpected error during rewrite: {e}", exc_info=True)
