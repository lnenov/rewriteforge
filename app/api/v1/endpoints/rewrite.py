...
from app.api.v1.models import RewriteRequest, RewriteResponse, Style
...
async def check_text_length(
    request_body: RewriteRequest = Body(...) # Get raw body access if needed, but Pydantic model is fine
):
    if len(request_body.text) > settings.MAX_TEXT_LENGTH:
        logger.warning(f"Rejected request due to excessive length: {len(request_body.text)} chars")
        raise HTTPException(
            status_code=400,
...
async def rewrite_text_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Rewrite service backend is unavailable.")
        ...
        logger.exception("An unexpected error occurred during text rewriting.") # Logs traceback
