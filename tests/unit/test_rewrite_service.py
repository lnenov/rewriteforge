...
def cached_rewrite_service(mock_llm_adapter):
    ...
    with patch("app.services.rewriter.settings.CACHE_ENABLED", True), patch(
        "app.services.rewriter.rewrite_cache", MagicMock()
    ):  # Mock the cache instance if needed
        ...
        if hasattr(
            service, "_rewrite_decorator"
        ):  # Check if using the complex method
            if hasattr(
                service._rewrite_decorator.__closure__[0].cell_contents,
                "cache_clear",
            ):
                service._rewrite_decorator.__closure__[
                    0
                ].cell_contents.cache_clear()
        yield service


@pytest.mark.asyncio
@patch(
    "app.services.rewriter.settings.CACHE_ENABLED", True
)  # Ensure cache is seen as enabled
...
async def test_rewrite_text_cache_hit(mock_cache, mock_llm_adapter):
    ...
    mock_cache.__contains__ = MagicMock(
        side_effect=lambda key: key in cache_store
    )
